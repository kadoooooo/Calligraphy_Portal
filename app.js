...
let currentPaperTab = "半紙"; // 現在選択されている紙の種類


// --- 時間（HH:MM）を比較用の数値（分）に変換する関数 ---
function parseTimeToMinutes(timeStr) {
    if (!timeStr) return 9999; // 時間がない場合は一番下に回す
...
document.querySelectorAll('.paper-tab').forEach(tab => {
    tab.addEventListener('click', (e) => {
        ...
        renderAttendanceSchedules(); // タブが押されたらスケジュールを再描画
    });
});
...
function renderAttendanceSchedules() {
    ...
    // 日付 と 紙の種類（現在のタブ）でフィルタリング
    let targetSchedules = schedules.filter(s => s.date === selectedDateStr && (s.paper || "半紙") === currentPaperTab);

    // ▼ 時間の早い順に並び替え（数値を計算して比較）▼
    ...
    targetSchedules.forEach(sch => {
        ...
        // 担当先生が配列であればカンマ区切り、そうでなければ文字列として表示（過去データ対応）
...
function openAttendanceForm(id, dateStr) {
    ...
    document.querySelector('.paper-tab-container').classList.add('hidden'); // フォームを開いたらタブも隠す
...
function editSchedule(id) {
    ...
    document.querySelectorAll('#schedule-teachers input').forEach(node => {
        // 過去データの互換性のため sch.teacher（文字列）との比較も含む
...
function resetAdminForm() {
    ...
    document.querySelectorAll('#schedule-teachers input').forEach(n => n.checked = false); // チェックリセット
...
function updatePrintSelect() {
    ...
    sorted.forEach(sch => {
        ...
        option.value = sch.id;
        option.textContent = `${sch.date} (${dayOfWeek}) ${sch.time} - ${sch.title || '通常稽古'} (${teachersStr}先生) [${sch.paper || '未設定'}]`;
